...
# Tabla Gestión Cultivo (con relaciones)
...
class UserManagementFrame(tk.Frame):
    ...
    def create_user(self):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("INSERT INTO usuarios (username, password, role, email) VALUES (?, ?, 'usuario', ?)", (username, password, email))
            conn.commit()
    ...
    def edit_user(self):
        ...
        try:
            conn = sqlite3.connect("cultivos.db")
            cursor = conn.cursor()
            cursor.execute("UPDATE usuarios SET username = ?, password = ?, email = ? WHERE username = ?", (new_username, new_password, new_email, username))
            conn.commit()
            conn.close()
